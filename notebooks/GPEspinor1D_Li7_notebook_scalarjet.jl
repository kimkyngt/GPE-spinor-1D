...
begin
	...
	gr()
	default_font = "Arial"
	default(titlefont = (10, default_font, :black), legendfont = (8, default_font, :black), guidefont = (10, default_font, :black), tickfont = (9, default_font, :black), framestyle = :box, size = (500, 300), dpi = 200, lw = 1)
end
...
begin
    ...
    fname = "scalar_q1.0kHz_seed10_N1.2e4"
...
begin
	...
	function get_psinoise(seed, trWigner, ψbec; spin0::Bool = false)
		...
		if trWigner
			...
			ϕin_p = Ket(bx, rand(MersenneTwister(randnum[1]), nx)*0)
			if spin0
				ϕin_0 = Ket(bx, rand(MersenneTwister(randnum[2]), nx))
			else
				ϕin_0 = Ket(bx, rand(MersenneTwister(randnum[2]), nx))
			end
			ϕin_m = Ket(bx, rand(MersenneTwister(randnum[3]), nx)*0)
			ψnoise = ϕin_p ⊕ ϕin_0 ⊕ ϕin_m	
			ψnoise = make_orthogonal(ψnoise, ψbec)
...
# ╔═╡ 6116625f-24a7-45ee-ab1f-d0fdda2b0569
md"### Heatmap visualization"

# ╔═╡ 61cece20-d78d-4999-ba34-b23eec867d94
T_dy[90]/ω⊥*1e3
...
# ╔═╡ 8ee6eca0-a0aa-4f94-9cc7-c99e958aecb1
	if token_quenchdy
		plt, n = plot_pop_dynamics(T_dy/ω⊥ *1e3, ψt_dy; yscale = :log10)
		plt
...
# ╔═╡ 498e45c5-ee38-48b8-83e3-566f35319c8c
begin
	using MAT
	tt = collect(T_dy/ω⊥ *1e3)
	ff = matopen(raw"G:\matfile.mat", "w")
	write(ff, "t", tt)
	write(ff, "n", n)
	close(ff)
end
...
# ╔═╡ c2c857d0-df77-4d2a-b222-fcf31d06b61b
plot_wfn(xx_um, ψt_dy[Tindx[1]])

# ╔═╡ 4d7ef911-50d5-45e1-8b0b-162ac872de7e
plot_wfn(xx_um, ψt_dy[Tindx[1]], ylims = (-.1, .5), xlims = (-30, 30); size = (400, 200), legend = false, ylabel = "")

# ╔═╡ 3c8ecd71-f759-47a1-b411-923e4af39dc9
plot_wfn(xx_um, ψt_dy[Tindx[2]])

# ╔═╡ 7f8cbc0b-b68d-493c-b1b1-e10c211db21b
plot_wfn(xx_um, ψt_dy[Tindx[3]])

# ╔═╡ 4804d762-57ed-46fd-86b1-79751b85a888
plot_wfn(xx_um, ψt_dy[Tindx[4]])

# ╔═╡ d6ff27ec-ea7d-47d9-9cdf-2d604eb4b1bd
ξsp = sqrt(1/(8*π*(-a2Li+a0Li)/3/(2*π*a⊥^2) * npeakSI))
