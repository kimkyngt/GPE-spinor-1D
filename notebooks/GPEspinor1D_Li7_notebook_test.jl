...
begin
	...
	xmaxSI 		= 300e-6	# maximum single-sided domain length [m]
...
function plot_snapshots_3D(xx, ψarray, T; kwargs...)
	...
	for kk = 1:3
		...
		for ii = 1:length(T)
			plot!(xx,fill(T[ii], length(xx)),(get_density(ψarray[ii])[kk]), c = kk, lw = 1, alpha = 0.8)
		end
	end
	plt
end
...
# ╔═╡ 243c33ee-6146-4f39-93c0-9be5103dd89a
plot_wfn(xx_um, ψt_dy[100])
