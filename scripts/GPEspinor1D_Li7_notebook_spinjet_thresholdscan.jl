...
"""
for atomnumber = range(1e3, 5e4, length = 100)

# ╔═╡ 55ff32a3-0e3d-4a66-8764-ce3fee1fd20c
begin
	# Input parameters in SI unit
    species     = "Li7"
    Natom 		= atomnumber		    # atom number
    f⊥ 			= 1.2e3        # [Hz]
    fSI			= 10.0         # [Hz]
	...
	trWigner 	= true # select type of the noise
	...
	function get_psinoise(seed, trWigner, ψbec; spin0::Bool = false)
		...
		if trWigner
			...
			if spin0
				...
				ϕin_0 = Ket(bx, generate_fluctuation(randnum[2])*0) 
			end
			...
			if spin0
				...
				ϕin_0 = Ket(bx, rand(MersenneTwister(randnum[2]), nx)*0)
			end
...
begin
	...
	if token_quenchdy
		...
		if token_save
			safesave(datadir("sims","Li7", "spinjet_Nscan", fname*".jld2") ,data_dynamics)
		end
	end
end
...
# ╔═╡ 8ee6eca0-a0aa-4f94-9cc7-c99e958aecb1
	if token_quenchdy
		plot_pop_dynamics(T_dy/ω⊥ *1e3, ψt_dy; yscale = :log10)
...
plot_wfn(xx_um, ψt_dy[Tindx[1]], ylims = (-1, 30), xlims = (-30, 30); size = (400, 200), legend = false, ylabel = "")
