...
Todo
- [ ] writting all the codes line by line again with more "quantum mechanical" way. Sperate the script name with "_v2"
=#
...
begin
    include("Fundamental_constants_SI.jl")
...
begin
    ...
    include("Dependent_constants_Li.jl")
    xmaxSI 		= 2*RTF
    dxSI 		= 2*xmaxSI/nx
...
begin
    # load functions
    include("Import_Functions_v2.jl");

    # construct single particle Hamiltonian
    include("Construct_Hamiltonian_v2.jl");

    # Preparing the initial state
    ψ0 =  initialstate_gaussian_spinor(bs, bx, Natom, σ = RTF/4/a⊥)
    plot_spinor_n(xx_um, ψ0)
...
begin
    ...
    safesave(datadir("sims","Li7",  fname*".bson") ,data_dynamics)
end
