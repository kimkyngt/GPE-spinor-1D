...
function plot_wfn(xx, ψ; kwargs...)
    # Plot wavefunction in SI units
    bs = ψ0.basis.bases[2]
    ρp = projector(Ket(bs, [1, 0, 0]))
    ρ0 = projector(Ket(bs, [0, 1, 0]))
    ρm = projector(Ket(bs, [0, 0, 1]))
    ...
    plot!(xx, n2,  label = "0", linewidth = 2, linestyle = :dash; kwargs...)
    plot!(xx, n3, label = "-1", linewidth = 2, linestyle = :dot; kwargs...)
...
function plot_wfn_phase(xx, ψ)
    ...
    plot!(xx, n3, label = "-1", linewidth = 2, linestyle = :dot)
    xlabel!("Position [μm]")
    ylabel!("Phase [rad]")
    return plt
end
...
function initialstate_gaussian_spinor(spinbasis, xbasis, Natom; σ = xbasis.xmax/4, spinratio = [1.0, 1.0, 1.0])
    ψ =  gaussianstate(xbasis, 0, 0, σ) ⊗ Ket(spinbasis, normalize!(spinratio))
    ψ = ψ *sqrt(Natom/dx)/norm(ψ)
    return ψ
end


function initialstate_uniform()
